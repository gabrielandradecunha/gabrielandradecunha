# Hello! 👋🤘

- 🎓 Computer Engineering Student
- 💻 Programmer and Web Developer  
- 🐧 Systems Administrator and DevOps Enthusiast

| Area | Stack |
|------|-----------------------------|
| 💻 **Programming Languages** | Java, PHP, Javascript, Python, C/C++ |
| 📚 **Frameworks & Libraries** | Laravel, Spring Boot, Express, React, FastAPI |
| 🏗️ **Architectures & Paradigms** | OOP, REST APIs, MVC, Microservices |
| ⚙️ **Infrastructure & DevOps** | Linux, Docker, Kubernetes, Jenkins, AWS, Nginx, Git |
| 🗄️ **Databases** | SQL, PostgreSQL, MySQL, SQLite |
| 🔌 **Microcontrollers & IoT** | ESP32, Arduino, MQTT, FreeRTOS |
